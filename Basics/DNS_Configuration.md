...
This guide describes **DNS Zones** — including Forward, Reverse, Primary, Secondary, and CNAME — with **step-by-step configuration instructions** and **illustrative screenshots**.  

---

## 🚀 Step 1: Launch Server Manager  
1. Click on the **Start Menu**.  
2. Choose **Server Manager**.  
...
- This is the most frequently used zone in DNS.  
- Common record types:  
  - **A Record** → Links domain to IPv4 address  
  - **AAAA Record** → Links domain to IPv6 address  
  - **CNAME Record** → Creates an alias for another record  
...
## 🛠️ Primary Zone  
- Holds the master (editable) copy of DNS records.  
- Acts as the authoritative source for its namespace.  
- Maintains **A, CNAME, MX, PTR** records.  
- Synchronizes with Secondary Zones.  

---

### ✅ How to Configure a Reverse Lookup Zone  
...
### 🔹 Description  
- A **read-only duplicate** of the Primary Zone.  
- Receives updates via **Zone Transfers**.  
- Adds **fault tolerance and load distribution**.  
- Cannot be modified directly.  

✅ Summary: **Backup of the Primary Zone (read-only)**  

---

### ✅ How to Configure Secondary Zone (Server2)  
...
2. Launch **DNS Tool → Forward Lookup Zones → New Zone**.  
...
3. Choose **Secondary Zone → Next → microsoft.com → Master IP 192.168.10.1 → Finish**.  
...
4. Initially, you might see an error message → **Zone not loaded**.  

<img width="1916" height="1002" src="https://github.com/user-attachments/assets/0d409fd2-37d6-4a13-ba4a-a6e8898fc012" />  

---

### ✅ Enable Zone Transfer (Server1)  
1. Right-click **microsoft.com → Properties → Zone Transfer**.  
2. Enable **Allow zone transfer → Only to servers listed below → Add IP 192.168.10.2**.  
...
3. Refresh the zone — records should now synchronize on Server2.  

<img width="1919" height="1010" src="https://github.com/user-attachments/assets/8bb3f019-f372-43b4-bb5e-3f963cd4e3a1" />  

👉 Reminder: Server2 cannot create or modify DNS records (read-only).  
...
### 🔹 Description  
- CNAME stands for **Canonical Name Record**, also known as an alias.  
- It maps **one hostname to another**.  
